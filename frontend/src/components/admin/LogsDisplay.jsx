...
const LogsDisplay = ({ filteredLogs, logs, logContainerRef }) => {
    ...
    const truncateTokens = (message) => {
        // Pattern to match JWT tokens (typically long base64 strings after 'token=')
        const tokenPattern = /token=([A-Za-z0-9._-]{50,})/g;
        return message.replace(tokenPattern, 'token=*****');
    };
    ...
    return (
        <div className="card-glass p-4 lg:p-6 border border-white/20 hover:border-emerald-200/50 transition-all duration-300">
            ...
            <div 
                ref={logContainerRef}
                className="max-h-80 sm:max-h-96 lg:max-h-[500px] xl:max-h-[600px] overflow-y-auto space-y-2 lg:space-y-3 pr-2"
                style={{
                    scrollbarWidth: 'thin',
            ...
            >
                {filteredLogs.length === 0 ? (
                    ...
                    filteredLogs.map((log, index) => (
                        <div 
                            key={index} 
                            className={`p-3 lg:p-4 rounded-lg border-l-4 transition-colors hover:shadow-sm ${
                        ...
                        >
                            <div className="flex items-start justify-between">
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center mb-1.5 flex-wrap gap-1">
                                    ...
                                    <div className="text-xs lg:text-sm text-gray-800 flex items-start">
                                        ...
                                        <span className="flex-1 break-words leading-relaxed">{truncateTokens(log.message)}</span>
                                    </div>
                                    {(log.module || log.function) && (
                                        <div className="text-xs text-gray-500 mt-1.5 break-words bg-gray-50 px-2 py-1 rounded">
