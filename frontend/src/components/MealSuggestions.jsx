  ...
  if (isMobile) {
    return (
      <div className="fixed inset-0 bg-white z-50 flex flex-col">
        {/* Mobile Header */}
        <div className="bg-gradient-to-r from-emerald-50 to-blue-50 border-b border-gray-200 px-4 py-3 flex items-center justify-between sticky top-0 z-10">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-emerald-600 rounded-full flex items-center justify-center">
              <HiOutlineSparkles className="w-4 h-4 text-white" />
            </div>
            <div>
              <h2 className="text-lg font-bold text-gray-800">Meal Suggestions</h2>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={isLoading}
              className="p-2 text-gray-500 hover:text-emerald-600 rounded-lg transition-colors"
            >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 pb-safe">
          {!hasTargets && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 mb-4">
              <div className="flex items-center gap-2 mb-2">
                <MdLightbulb className="w-4 h-4 text-amber-600" />
                <h3 className="font-medium text-amber-800 text-sm">Set Nutritional Targets</h3>
              </div>
          ...
          {/* Mobile Remaining Targets */}
          {hasTargets && Object.keys(remainingTargets).length > 0 && (
            <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3 mb-4">
              <h3 className="font-medium text-emerald-800 mb-2 text-sm">Remaining Daily Targets</h3>
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="flex justify-between">
                  <span className="text-emerald-600 font-medium">Calories:</span>
                  <span className="text-emerald-800">{Math.round(remainingTargets.calories || 0)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-emerald-600 font-medium">Protein:</span>
                  <span className="text-emerald-800">{Math.round(remainingTargets.protein || 0)}g</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-emerald-600 font-medium">Carbs:</span>
                  <span className="text-emerald-800">{Math.round(remainingTargets.carbs || 0)}g</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-emerald-600 font-medium">Fat:</span>
                  <span className="text-emerald-800">{Math.round(remainingTargets.fat || 0)}g</span>
                </div>
              </div>
            </div>
          )}
          ...
          {isLoading && (
            <div className="flex items-center justify-center py-8">
              <div className="text-center">
                <MdRefresh className="animate-spin h-6 w-6 text-emerald-500 mx-auto mb-2" />
          ...
          {/* Mobile No Suggestions State */}
          {!isLoading && !error && suggestions.length === 0 && hasTargets && (
            <div className="text-center py-8">
              <div className="w-12 h-12 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-3">
                <MdLightbulb className="w-6 h-6 text-gray-400" />
              </div>
              <h3 className="text-base font-medium text-gray-800 mb-2">No Suggestions Available</h3>
        ...
        <div className="p-6 overflow-y-auto" style={{ maxHeight: 'calc(90vh - 80px)' }}>
          {!hasTargets && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6">
              <div className="flex items-center gap-2 mb-2">
                <MdLightbulb className="w-5 h-5 text-amber-600" />
                <h3 className="font-medium text-amber-800">Set Nutritional Targets</h3>
              </div>
          ...
          {/* Desktop Remaining Targets Summary */}
          {hasTargets && Object.keys(remainingTargets).length > 0 && (
            <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 mb-6">
              <h3 className="font-medium text-emerald-800 mb-3">Remaining Daily Targets</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                <div>
                  <span className="text-emerald-600 font-medium">Calories:</span>
                  <span className="ml-1 text-emerald-800">{Math.round(remainingTargets.calories || 0)} kcal</span>
                </div>
                <div>
                  <span className="text-emerald-600 font-medium">Protein:</span>
                  <span className="ml-1 text-emerald-800">{Math.round(remainingTargets.protein || 0)}g</span>
                </div>
                <div>
                  <span className="text-emerald-600 font-medium">Carbs:</span>
                  <span className="ml-1 text-emerald-800">{Math.round(remainingTargets.carbs || 0)}g</span>
                </div>
                <div>
                  <span className="text-emerald-600 font-medium">Fat:</span>
                  <span className="ml-1 text-emerald-800">{Math.round(remainingTargets.fat || 0)}g</span>
                </div>
              </div>
            </div>
          )}
          ...
          {isLoading && (
            <div className="flex items-center justify-center py-12">
              <div className="text-center">
                <MdRefresh className="animate-spin h-8 w-8 text-emerald-500 mx-auto mb-2" />
                <p className="text-gray-600">Generating personalized suggestions...</p>
              </div>
            </div>
          )}

          {/* Desktop Suggestions Grid */}
          {!isLoading && suggestions.length > 0 && (
