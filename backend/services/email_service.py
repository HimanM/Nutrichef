...
class EmailService:
    ...
    def send_verification_email(self, user_email, user_name, token):
        ...
        html_body = f"""
        <p>Hello {user_name},</p>
        <p>Welcome to NutriChef! Please click the link below to verify your email address:</p>
        <p><a href="{verification_url}">{verification_url}</a></p>
        <p>If you did not create an account, please ignore this email.</p>
        <p>Thanks,</p>
        <p>The NutriChef Team</p>
        """
    ...
    def send_reply_to_contact_message(self, recipient_email, recipient_name, subject, reply_body):
        """
        Sends a reply email to a user who submitted a contact message.
        """
        mail = current_app.extensions.get('mail')
        if not mail:
            current_app.logger.error("Flask-Mail extension not found or not initialized.")
        ...
        html_body = f"""
                ...
                <p>Thank you for contacting NutriChef. Here is our reply regarding your message:</p>
                <hr>
                <p>{reply_body_formatted}</p>
                <hr>
                <p>If you have any further questions, please feel free to reach out to us again.</p>
                <p>Sincerely,</p>
                <p>The NutriChef Support Team</p>
                """
